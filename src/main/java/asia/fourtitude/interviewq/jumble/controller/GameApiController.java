...
import asia.fourtitude.interviewq.jumble.model.GameSession;
...
public class GameApiController {
        ...
        public GameApiController(JumbleEngine jumbleEngine, GameSessionRepository gameSessionRepository) {
        ...
        public ResponseEntity<GameGuessOutput> newGame() {
                ...
                session.setModifiedAt(model.getModifiedAt());
                session.setOriginalWord(gameState.getOriginal());
                session.setScrambleWord(gameState.getScramble());
                session.setSubWords(gameState.getSubWords());
        ...
        public ResponseEntity<GameGuessOutput> playGame(
                 ...
                 */
                if (input == null || input.getId() == null || input.getId().trim().isEmpty()) {
                        output.setResult("Invalid Game ID.");
                        return new ResponseEntity<>(output, HttpStatus.NOT_FOUND);
                }

                GameSession session = null;
                Optional<GameSession> sessionOpt = this.gameSessionRepository.findById(input.getId());
                if (sessionOpt.isPresent()) {
                        session = sessionOpt.get();
                }

                GameGuessModel model = this.gameBoards.get(input.getId());
                if (model == null && session != null) {
                        GameState gameState = new GameState(session.getOriginalWord(), session.getScrambleWord(), session.getSubWords());
                        model = new GameGuessModel();
                        model.setId(session.getId());
                        model.setCreatedAt(session.getCreatedAt());
                        model.setModifiedAt(session.getModifiedAt());
                        model.setGameState(gameState);
                        this.gameBoards.put(session.getId(), model);
                }
                ...
                String result;
                if (gameState.getSubWords().containsKey(guessWord)) {
                        if (gameState.getSubWords().get(guessWord) == Boolean.TRUE) {
                                result = "Guessed already.";
                        } else {
                ...
                session.setModifiedAt(model.getModifiedAt());
                session.setSubWords(model.getGameState().getSubWords());
                this.gameSessionRepository.save(session);
