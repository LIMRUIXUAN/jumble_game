...
class JumbleEngineTest {
    ...
    JumbleEngine engine = new JumbleEngine();
    ...
    @Test
    void givenValidSuffix_thenSomeWords() {
        Collection<String> words = engine.wordsMatchingSuffix("ing");
        assertTrue(words.size() > 0, "suffix=ing");
        assertTrue(words.contains("amazing") || words.contains("boring") || words.contains("caring"),
                "suffix=ing should contain common matching words");
        assertEquals(0, engine.wordsMatchingSuffix("!").size(), "suffix=<PUNCT>");
    }
